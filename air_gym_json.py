...
opuss = []
print(
    'Введите одну из следующих категорий: батуты для дачи, шведские стенки, детские шведские стенки, спортивные тренажёры')
...
while count <= 20:
    ...
    for i in heads:
        ...
        storage.append({'name': zagol, 'cena': price, 'code': get_code, 'param_1': charact_1,
                        'param_2': charact_2, 'param_3': charact_3, 'param_4': charact_4, 'param_5': charact_5,
                        'photo': photo, 'URL': ssylka})
        try:
            with open(f'{filename}.json', 'r', encoding='utf-16') as f:
                existing_data = json.load(f)
        except FileNotFoundError:
            existing_data = []
        # Добавление новых данных к существующим
        existing_data.extend(storage)
        with open(f'{filename}.json', 'w', encoding='utf-16') as f:
